class draggable_elements {
    ...
    if (window.MutationObserver) {
          ...
          if (
            ...
            if (mutation.attributeName === "class") {
              ...
              theseNodes = [
                mutation.target,
                ...childrenArray
              ]; /*console.log("mutation.target",mutation.target,"childrenArray",childrenArray);console.log("these Nodes",theseNodes);*/
            } else {
              theseNodes =
                mutation.addedNodes; /* console.log("AddedNodes",mutation.addedNodes);*/
            }
      ...
      observer.observe(document.body, {
        ...
        attributeFilter: [
          "class",
          "id"
        ] /* if you want to trigger off dynamic changes to more attributes, add them here */
      });
  ...
  makeDraggable(element, config) {
    ...
    element.setAttribute(
      "ktwp-de-rect-x",
      rect.x + getComputedStyle(element)["display"] == "fixed"
        ? 0
        : window.scrollX
    );
    element.setAttribute(
      "ktwp-de-rect-y",
      rect.y + getComputedStyle(element)["display"] == "fixed"
        ? 0
        : window.scrollY
    );
    element.setAttribute("data-draggable", "true");
    element.setAttribute(
      "data-ktwp-de-position",
    ...
    element.title =
      ...
      "☝ Drag me " +
      (constraintDesc[config.constraint] &&
      constraintDesc[config.constraint].desc
        ? constraintDesc[config.constraint].desc + " "
        : "") +
      "to reposition!";
    if (
      constraintDesc[config.constraint] &&
      constraintDesc[config.constraint].class
    ) {
    ...
    if (
      ...
      if (getComputedStyle(theDragElement).position == "static") {
        element.style.position =
          "relative"; /* need this so newDiv is correctly sized and positioned. */
      }
    }
    ...
    if (config.constraint === "corners") {
      ...
      element.style.zIndex =
        "9999"; /* moved from handleMouseDown just for corner-snapped elements; see note about it in that function. Ideally, eventually, it should check if the developer has set this with getComputedStyles and only assign it if not; but I don't need that level of detail right now, so, will procrastinate on it. */
    ...
    element.addEventListener("touchcancel", (e) => this.handleTouchEnd(e), {
      passive: false
    }); // handle when a touch is interrupted
    ...
    if (
      element.id &&
      sessionStorage.getItem("ktwp-de-elem-" + element.id + "-x")
    ) {
      const absoluteX = sessionStorage.getItem(
        "ktwp-de-elem-" + element.id + "-x"
      );
      const absoluteY = sessionStorage.getItem(
        "ktwp-de-elem-" + element.id + "-y"
      );
      const absoluteR = sessionStorage.getItem(
        "ktwp-de-elem-" + element.id + "-r"
      );
      const absoluteB = sessionStorage.getItem(
        "ktwp-de-elem-" + element.id + "-b"
      );
      //console.log("ss",sessionStorage);
      this.handleMouseDown({
        /* prepare element for dragging (create placeholder if nec, etc) */
        button: 0, // Left click
        target: element,
        clientX: rect.left,
        clientY: rect.top,
  ...
  handleMouseDown(e) {
    ...
    if (
      ...
      getComputedStyle(element).position !=
        "absolute" /*previously used element.style.position, but this doesn't pick up attributes defined in css stylesheets, only on the element. */
    ) {
      var newNode = document.createElement("div");
      var nodeUUID = "10000000-1000-4000-8000-100000000000".replace(
        /[018]/g,
        (c) =>
          (
            +c ^
    ...
    if (newNode || getComputedStyle(element).position == "absolute") {
      ...
      document.body.appendChild(
        element
      ); /* if the element wasn't fixed, move it to a child of the body, because CSS transforms on an ancestor (or similar things) can create a new stacking context, and "fixed at 0,0" follows the stacking context root element, not necessarily the body. If it was already fixed, we'll leave it to the original page code to put it in the context it should be in. */
    }
...
END old handlers */
  ...
  handleTouchMove(e) {
    ...
    if (!this.dragStarted) {
      // Only decide if it's a drag if it hasn't been confirmed yet
      if (
        deltaX > this.touchMovementThreshold ||
        deltaY > this.touchMovementThreshold
      ) {
        ...
        this.activeElement.classList.add(
          "ktwp-de-disablehover"
        ); /* so hover state doesn't trigger after drag */
    ...
    const simulatedMouseEvent = {
      ...
      movementX:
        currentTouchX -
        (this._lastTouchX !== undefined ? this._lastTouchX : currentTouchX),
      movementY:
        currentTouchY -
        (this._lastTouchY !== undefined ? this._lastTouchY : currentTouchY),
      preventDefault: () => {}
    };
  ...
  handleCornerMovement(e) {
    ...
    const elementHeight = this.activeElement.getAttribute(
      "ktwp-de-rect-height"
    ); //rect.height
  ...
  handleVerticalMovement(e) {
    ...
    if (this.activeElement.style.position != "fixed") {
      this.activeElement.style.left =
        rect.left +
        "px"; /* otherwise will jump when set to fixed, if page has been scrolled */
    }
  ...
  handleMouseUp(e) {
    ...
    if (this.activeElement.id && !e.simulated) {
      /*if the element has an ID and is not fixed or corner-constrained, store absolute X & Y */
      sessionStorage.setItem(
        "ktwp-de-elem-" + this.activeElement.id + "-x",
        this.activeElement.style.left
      );
      sessionStorage.setItem(
        "ktwp-de-elem-" + this.activeElement.id + "-y",
        this.activeElement.style.top
      );
      sessionStorage.setItem(
        "ktwp-de-elem-" + this.activeElement.id + "-r",
        this.activeElement.style.right
      );
      sessionStorage.setItem(
        "ktwp-de-elem-" + this.activeElement.id + "-b",
        this.activeElement.style.bottom
      );
    }
